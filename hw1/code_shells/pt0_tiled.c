 ...
 * Assignment 1: Optimization Techniques with loop tiling
 */
#include <stdio.h>
#include <stdlib.h>
#include <sys/time.h>
...
void count_array(int size, char ** char_array)
{
  ...
  */
  ...
  // The 2nd attempt at tiling
  // This is the best speed up
  for ( i = 0; i < size; i+=4) {
    for ( j = 0; j < STRING_SIZE; j+=4) {
      theChar = char_array[i][j];
      charLoc = ((int) theChar) - 97;
  ...
  /*
  // The 3rd attempt at tiling
  // This is the worst speed up
  for ( i = 0; i < size; i+=4) {
    for ( j = 0; j < STRING_SIZE; j+=4) {
      for ( k = i; k < i+4; k++) {
        for ( l = j; l < j+4; l++) {
